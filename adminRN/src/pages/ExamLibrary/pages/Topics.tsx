...
const Topics: React.FC = () => {
  ...
  const fetchSubjects = useCallback(
    debounce(async (force = false) => {
      ...
      if (!force && optionCacheRef.current.has(cacheKey)) {
  ...
  const fetchChapters = useCallback(
    async (subjectId: string, callback: (options: Option[]) => void, retryCount = 0) => {
      ...
      if (optionCacheRef.current.has(cacheKey)) {
        const cached = optionCacheRef.current.get(cacheKey);
      ...
      try {
        ...
        if (modalMode === 'edit' && selectedItem) {
          const chapterId = typeof selectedItem.chapterId === 'string' 
            ? selectedItem.chapterId 
            : selectedItem.chapterId?._id;
          if (chapterId && options.some(opt => opt.id === chapterId)) {
            setTimeout(() => {
              formikRef.current?.setFieldValue('chapterId', chapterId);
            }, 0);
          }
        }
      } catch (error: any) {
  ...
  const handleDependentUpdate = useCallback(
    (subjectId: string, setFieldValue?: (field: string, value: any) => void) => {
      if (!subjectId || subjectId === prevSubjectIdRef.current) {
        if (!subjectId && setFieldValue) {
  ...
  const Effect: React.FC<EffectProps> = React.memo(
    ({ subjectId, handleDependentUpdate, isSubmitting, modalMode, selectedItem }) => {
      ...
      useEffect(() => {
        if (!isSubmitting && subjectId) {
          if (modalMode === 'edit' && initialLoad.current) {
      ...
      }, [subjectId, handleDependentUpdate, isSubmitting, modalMode]);
    ...
    ({ form, field, options = [], label = '', disabled = false, error = false }) => {
      ...
      useEffect(() => {
        if (field.value !== undefined && field.value !== null) {
          ...
          if (JSON.stringify(selected) !== JSON.stringify(myValue)) {
            setMyValue(selected);
          }
        }
      }, [field.value, options]);

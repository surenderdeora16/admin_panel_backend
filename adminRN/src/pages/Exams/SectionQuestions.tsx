...
const SectionQuestions = () => {
    ...
    try {
      ...
      if (sectionsRes.data?.status) {
        const sectionData = Array.isArray(sectionsRes.data.data.record)
          ...
          : sectionsRes.data.data.subjects?.find((s: any) => s._id === sectionId)
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      ...
      try {
        const res = await AxiosHelper.getData(`/question-selection/subjects/${taxonomy.selectedSubject}/chapters`)
        if (res.data?.status)
          setTaxonomy((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      ...
      try {
        const res = await AxiosHelper.getData(`/question-selection/chapters/${taxonomy.selectedChapter}/topics`)
        if (res.data?.status)
          setTaxonomy((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        setLoading(true)
        const res = await AxiosHelper.getData(`/question-selection/topics/${taxonomy.selectedTopic}/questions`, {
  ...
  }, [taxonomy.selectedTopic, pagination.page, pagination.limit, taxonomy.searchQuery])

  // Fetch section questions
  const fetchSectionQuestions = useCallback(async () => {
    try {
      setLoading(true)
      const res = await AxiosHelper.getData(`/test-series/${testSeriesId}/sections/${sectionId}/questions`, {
        page: sectionQPagination.page,
        limit: sectionQPagination.limit,
      })
      if (res.data?.status) {
        setSectionQuestions(res.data.data.record)
        setSectionQPagination((prev) => ({ ...prev, total: res.data.data.count }))
      }
    } catch (error) {
  ...
  }, [testSeriesId, sectionId, sectionQPagination.page, sectionQPagination.limit])
  ...
  const handleSelectAll = () => {
    setSelectedQuestions((prev) => (prev.length === questions.length ? [] : questions.map((q) => q._id)))
  }
  ...
  const handleAddQuestions = async () => {
    ...
    try {
      ...
      const res = await AxiosHelper.postData(`/test-series/${testSeriesId}/sections/${sectionId}/questions`, {
  ...
  const handleRemoveQuestion = async (questionId: string) => {
    try {
      ...
      const res = await AxiosHelper.deleteData(`/test-series/${testSeriesId}/sections/${sectionId}/questions`, {
  ...
  const customSelectStyles = {
    ...
    option: (base: any, state: any) => ({
      ...base,
      backgroundColor: state.isSelected ? "#fecdd3" : state.isFocused ? "#fee2e2" : "white",
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-8 space-y-4">
          <div className="flex items-center gap-2 text-slate-500 text-sm">
            <Link to="/test-series" className="hover:text-rose-600 transition-colors">
              Test Series
            </Link>
            <span className="text-slate-300">/</span>
            <Link to={`/test-series/${testSeriesId}/sections`} className="hover:text-rose-600 transition-colors">
              Sections
            </Link>
            <span className="text-slate-300">/</span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-1">Manage questions for this section</p>
            </div>
            <Link
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="p-5 bg-gradient-to-r from-rose-50 to-rose-100 border-b border-rose-200">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-slate-600 mt-1">Select questions to add to this section</p>
                </div>
                <button
              ...
              <div className="p-5 space-y-5 border-b border-slate-200 bg-slate-50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <ReactSelectInput
                    ...
                    value={
                      taxonomy.selectedSubject
                        ? subjectOptions.find((option) => option.value === taxonomy.selectedSubject) || null
                        : null
                    }
                    onChange={(option: SelectOption | null) =>
                      setTaxonomy((prev) => ({ ...prev, selectedSubject: option?.value || null }))
                    }
                  ...
                  <ReactSelectInput
                    ...
                    value={
                      taxonomy.selectedChapter
                        ? chapterOptions.find((option) => option.value === taxonomy.selectedChapter) || null
                        : null
                    }
                    onChange={(option: SelectOption | null) =>
                      setTaxonomy((prev) => ({ ...prev, selectedChapter: option?.value || null }))
                    }
                  ...
                  <ReactSelectInput
                    ...
                    value={
                      taxonomy.selectedTopic
                        ? topicOptions.find((option) => option.value === taxonomy.selectedTopic) || null
                        : null
                    }
                    onChange={(option: SelectOption | null) =>
                      setTaxonomy((prev) => ({ ...prev, selectedTopic: option?.value || null }))
                    }
                ...
                <SearchInput
                  ...
                  onChange={(e) => setTaxonomy((prev) => ({ ...prev, searchQuery: e.target.value }))}
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                >
                  {processing.adding ? (
                    ...
                    <>
                      <FiPlus className="text-lg" /> Add Selected ({selectedQuestions.length})
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-5 bg-gradient-to-r from-emerald-50 to-emerald-100 border-b border-emerald-200">
              <h2 className="text-xl font-bold text-slate-800">Section Questions</h2>
              <div className="flex items-center gap-2 mt-1">
                <FiGrid className="text-emerald-600" />
                <p className="text-slate-600">
                  Current questions: <span className="font-semibold text-emerald-700">{sectionQPagination.total}</span>
                </p>
              </div>
            </div>
            ...
            {sectionQuestions.length > 0 && (
              <div className="p-4 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-sm text-slate-600">
                  Showing {(sectionQPagination.page - 1) * sectionQPagination.limit + 1} to{" "}
                  {Math.min(sectionQPagination.page * sectionQPagination.limit, sectionQPagination.total)} of{" "}
                  {sectionQPagination.total}
                </div>
                <PaginationControls
                  {...sectionQPagination}
                  onPrev={() => setSectionQPagination((prev) => ({ ...prev, page: Math.max(1, prev.page - 1) }))}
                  onNext={() => setSectionQPagination((prev) => ({ ...prev, page: prev.page + 1 }))}
                />
              </div>
            )}
          </div>
...
const QuestionTable: React.FC<QuestionTableProps> = ({ questions, selected, onSelect, onSelectAll, loading }) => (
              ...
              >
                <td className="px-6 py-4">
                  <div className="flex items-center">
                    ...
                    >
                      <FiCheck className={`w-3.5 h-3.5 ${isSelected ? "opacity-100" : "opacity-0"}`} />
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="text-sm text-slate-800 font-medium line-clamp-2">{question.questionText}</div>
                </td>
                <td className="px-6 py-4">
                    ...
                    {[1, 2, 3, 4, 5].map((opt) => {
                      ...
                      return (
                        <span
                          ...
                          title={optionValue}
                        >
                          {opt}. {optionValue.length > 15 ? `${optionValue.substring(0, 15)}...` : optionValue} 
                        </span>
                      )
                    })}
                  </div>
                ...
                <td className="px-6 py-4">
                  <div className="text-sm font-medium text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md inline-block">
                    {question[question.rightAnswer as keyof Question] as string}
                  </div>
                </td>
              </tr>
            )
  ...
  <div className="overflow-y-auto max-h-[600px]">
    {loading ? (
      ...
      <div className="p-10 text-center">
        ...
        <p className="text-slate-400 text-sm mt-1">Use the Question Bank to add questions</p>
      </div>
    ) : (
      <table className="min-w-full divide-y divide-slate-200">
        <thead className="bg-slate-50 sticky top-0 z-10">
        ...
        <tbody className="bg-white divide-y divide-slate-200">
          {questions.map((item, index) => (
            <tr key={item._id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="px-6 py-4">
                <div className="text-sm font-medium text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md inline-block">
                  {item.questionId[item.questionId.rightAnswer as keyof Question] as string}
                </div>
              </td>
              <td className="px-6 py-4">
                <button
                ...
                >
                  {removingId === item.questionId._id ? <Spinner /> : <FiTrash2 />}
                  Remove
                </button>
              </td>
            </tr>
...
const PaginationControls: React.FC<PaginationControlsProps> = ({ page, total, limit, onPrev, onNext }) => {

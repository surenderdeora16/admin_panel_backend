...
exports.getNotesBySubject = async (req, res) => {
  try {
    ...
    const purchasedExamPlanIds = new Set(
      userPurchases.map((purchase) => purchase.itemId.toString())
    );
    ...
    const notesWithAccessInfo = notes.map((note) => {
      ...
      if (note.isFree) {
        ...
        const hasExamPlanAccess = purchasedExamPlanIds.has(
          note.examPlanId._id.toString()
        );
        ...
        if (hasExamPlanAccess) {
          const purchase = userPurchases.find(
            (p) => p.itemId.toString() === note.examPlanId._id.toString()
          );
    ...
    logger.error(`Error in getNotesBySubject: ${error.message}`, {
      error,
      userId: req.user?._id,
    });
...
exports.getNotesByExamPlan = async (req, res) => {
  try {
    ...
    logger.error(`Error in getNotesByExamPlan: ${error.message}`, {
      error,
      userId: req.user?._id,
    });
    return res.someThingWentWrong(error);
...
exports.downloadNote = async (req, res) => {
  try {
    ...
    if (note.isFree) {
      ...
      if (!purchase) {
        return res.status(403).json({
          status: false,
          message:
            "You need to purchase the associated exam plan to access this note",
          data: {
            noteId: note._id,
            title: note.title,
    ...
    logger.error(`Error in downloadNote: ${error.message}`, {
      error,
      userId: req.user?._id,
    });
    return res.someThingWentWrong(error);
...
exports.getSubjectsWithNotesCountByExamPlan = async (req, res) => {
  try {
    ...
    logger.error(
      `Error in getSubjectsWithNotesCountByExamPlan: ${error.message}`,
...
exports.getAllNotes = async (req, res) => {
  try {
    ...
    logger.error(`Error in getAllNotes: ${error.message}`, {
      error,
      userId: req.user?._id,
    });
    return res.someThingWentWrong(error);

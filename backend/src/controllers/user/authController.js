...
exports.register = async (req, res) => {
    try {
        ...
        const user = await User.create({ name, email, mobile, password: confirmPassword, state, district });
        ...
        return res.success(record, "Registration successful! Welcome to our platform.");
    } catch (error) {
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        if (!user) return res.noRecords("We couldn't find an account associated with this mobile number. Please check and try again.")
        ...
        return res.json({
            status: true,
            message: `${otp} OTP sent successfully to your mobile number. Please check and enter the OTP to reset your password.`,
            expiresAt: expiryTime
        });
        ...
        if (error.message === "Failed to send SMS. Please try again later.") {
            return res.status(400).json({ status: false, message: "Unable to send OTP at the moment. Please try again later." });
...
exports.verifyOtp = async (req, res) => {
    try {
        ...
        const otpRecord = await UserOTP.findOne({ mobile, isUsed: false }).sort({ createdAt: -1 })

        console.log("otpRecord", otpRecord)
        if (!otpRecord) {
            return res.status(400).json({ status: false, message: "No active OTP found. Please request a new OTP." });
        }
 ...
 if (!decoded || !decoded.mobile) {
    ...
    return res.status(401).json({
     ...
     message: "Invalid password reset session. Please request a new OTP."
        ...
        return res.success([], "Password has been reset successfully. You can now login with your new password.")
...
exports.changePassword = async (req, res) => {
    try {
        ...
        if (!isMatch) return res.status(401).json({ status: false, message: "Old password is incorrect. Please try again." });

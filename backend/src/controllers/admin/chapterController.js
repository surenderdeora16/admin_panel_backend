...
exports.getChapters = async (req, res) => {
  try {
    const { limit = 10, pageNo = 1, query = "", subjectId = null, orderBy = "name", orderDirection = 1 } = req.query
    ...
    return res.pagination(chaptersWithCounts, totalCount, limit, Number(pageNo))
  } catch (error) {
...
exports.updateChapter = async (req, res) => {
  try {
    ...
    if (name || subjectId) {
      ...
      if (name) conflictFilter.$or.push({ 
...
exports.deleteChapter = async (req, res) => {
  try {
    ...
    if (topicCount > 0) {
      return res.status(409).json({
        status: false,
        message: "Cannot delete chapter with active topics",
      })
    }

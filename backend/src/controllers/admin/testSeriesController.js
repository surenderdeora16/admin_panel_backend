...
exports.getTestSeriesById = async (req, res) => {
  try {
    ...
    const sections = await Section.find({ testSeriesId: req.params.id }).sort({ sequence: 1 })
...
exports.updateTestSeries = async (req, res) => {
  try {
    ...
    if (passingPercentage !== undefined) testSeries.passingPercentage = passingPercentage
    if (instructions !== undefined) testSeries.instructions = instructions
    if (isFree !== undefined) testSeries.isFree = isFree === "true" || isFree === true
...
exports.getTestSeriesSections = async (req, res) => {
  try {
    ...
    // Get sections
    const sections = await Section.find({ testSeriesId }).sort({ sequence: 1 })
...
exports.deleteSection = async (req, res) => {
  try {
    ...
    const questionCount = await TestSeriesQuestion.countDocuments({ sectionId })

    if (questionCount > 0) {
      return res.status(400).json({
        status: false,
        message: "Cannot delete section with associated questions. Please remove the questions first.",
      })
    }
...
exports.getSectionQuestions = async (req, res) => {
  try {
    ...
    // Count total records
    const total = await TestSeriesQuestion.countDocuments({ testSeriesId, sectionId })
    ...
    const testSeriesQuestions = await TestSeriesQuestion.find({ testSeriesId, sectionId })
      .populate({
        path: "questionId",
...
exports.addQuestionsToSection = async (req, res) => {
  try {
    ...
    const highestSequence = await TestSeriesQuestion.findOne({ testSeriesId, sectionId })
      .sort({ sequence: -1 })
      .select("sequence")
...
exports.getQuestionSelectionData = async (req, res) => {
  try {
    ...
    const subjects = await Subject.find({ status: true, deletedAt: null }).select("_id name").sort({ name: 1 })
...
exports.getQuestionsByTopic = async (req, res) => {
  try {
    ...
    const questions = await Question.find(queryObj)
      .select("_id questionText option1 option2 option3 option4 option5 rightAnswer")
      .skip((pageNo - 1) * limit)
      .limit(limit)
      .sort({ createdAt: -1 })
